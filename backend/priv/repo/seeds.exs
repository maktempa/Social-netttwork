...
Backend.User.create(%{
  login: "name6",
  password: "Ppassword1",
  password_confirmation: "Ppassword1",
  gender: "male"
})
...
fr =
  Backend.Repo.all(
    from(u in Backend.User,
      join: f in Backend.Friendship,
      where: u.id == f.requester_user_id or u.id == f.respondent_user_id
    )
  )
